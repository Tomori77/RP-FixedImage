...
for (const marker of [
    ...
    "&character_name=' + encodeURIComponent(currentCharacter.value?.name || '未命名角色') + '&model=",
    "'tag=$1&',",
    "encodeURIComponent(String(prompt || '').trim())"
]) {
    if (!appSource.includes(marker)) throw new Error(`assets/js/app.js is missing image patch marker: ${marker}`);
}
...
console.log('RP-Hub 1.7.6 static baseline and image patch check passed');
